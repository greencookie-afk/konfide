...
export default function ExploreHero({ filters, total }: ExploreHeroProps) {
  const hasFilters = Boolean(filters.query || filters.topic || filters.sort !== "soonest");
    ...
    <section className="rounded-[18px] border border-on-surface/5 bg-surface-container-lowest p-5 shadow-sm sm:p-6">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-2xl">
          ...
          <h1 className="text-3xl font-bold tracking-tight text-on-surface md:text-4xl">Find a listener and book from real availability.</h1>
          <p className="mt-3 text-sm leading-6 text-on-surface-variant md:text-base">
            Published listeners appear here once their profile and weekly schedule are ready. Four compact cards per
            page keep browsing fast.
          </p>
        </div>
        <div className="rounded-[16px] border border-on-surface/5 bg-surface px-4 py-4 text-sm text-on-surface-variant">
          <span className="font-semibold text-on-surface">{total}</span> listener{total === 1 ? "" : "s"} ready to book
        </div>
      </div>
      ...
      <form action="/explore" method="GET" className="mt-5 flex flex-col gap-3 md:flex-row">
        ...
        {filters.sort !== "soonest" ? <input type="hidden" name="sort" value={filters.sort} /> : null}
