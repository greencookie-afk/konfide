...
import { getPathForRole } from "@/server/auth/paths";
import { createSignedToken, setSessionCookie, verifySignedToken } from "@/server/auth/session";
...
export async function finishGoogleAuth(request: Request) {
  ...
  try {
    ...
    const response = NextResponse.redirect(new URL(getPostAuthPath(user.role), getBaseUrl(request)), {
      headers: {
        "Cache-Control": "no-store",
      },
    });
